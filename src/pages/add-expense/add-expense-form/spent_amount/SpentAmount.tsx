...
const SpentAmount: React.FC<MyComponentProps> = ({
  spentAmount,
  setSpentAmount,
}) => {
  ...
  return (
    <div className={style.expesne_amount_input_div}>
      <label className={style.spend_amount_label}>
        Spend Amount:
        {spentAmount && (
          <DataDisplayCard css={{ backgroundColor: "#e85d04" }}>
            {spentAmount}
          </DataDisplayCard>
        )}
      </label>
        ...
        {suggestion_data.map((val, index) => {
          return (
            <div
              ...
              className={style.expense_suggestion}
