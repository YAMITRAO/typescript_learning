 ...
 console.log("data is :- ",data)
  ...
  const postData =  async (e:React.FormEvent<HTMLFormElement>) => {
    ...
    let payerPost_promise = axiosInstance.post(`${payer}`, {
     ...
     try{
      let response =await Promise.all([...payeePostArr_promise, payerPost_promise])
  ...
  return (
    <div className={style.add_expense_form_container}>
      ...
      <form className={style.add_expense_form} onSubmit={(e)=> {
        postData(e)
      }}>
        <SpentAmount spentAmount={spentAmount} setSpentAmount={setSpentAmount} />
        ...
        <button className={style.expense_submit_button} type="submit">Submit</button>
      </form>
    </div>
  )
}
